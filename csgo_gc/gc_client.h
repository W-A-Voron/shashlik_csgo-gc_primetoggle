...
// ---------------------------------------------------------
// ОБРАБОТЧИК КОНЦА МАТЧА (ИСПРАВЛЕНА ОШИБКА)
// ---------------------------------------------------------
...
void ClientGC::OnMatchEndRunRewardDrops(GCMessageRead &messageRead)
{
    ...
    // В этом протобуфе нет поля winner. Вместо этого проверяем match_ending_results
    // Обычно 1 = победа, 2 = поражение (но зависит от игры). Мы будем считать победу, если result = 1.
    // Если у тебя матчи засчитываются как поражения, поменяй условие.
