...
// ===== КОНСТАНТЫ ТОЧНОЙ МЕХАНИКИ CS:GO =====
const int XP_PER_LEVEL = 5000;       // 5000 XP на каждый уровень аккаунта
const int WINS_PER_RANK = 10;        // 10 побед на каждое звание
const int XP_WIN = 300;              // XP за победу
const int XP_LOSE = 150;             // XP за поражение
const int XP_DAILY_BONUS = 250;      // Ежедневный бонус за первую победу
...
void ClientGC::HandleMessage(uint32_t type, const void *data, uint32_t size)
{
    ...
    if (messageRead.IsProtobuf())
    {
        ...
        // ===== КОНЕЦ МАТЧА (точная механика CS:GO) =====
...
void ClientGC::SaveRanksToConfig()
{
    ...
    ranksKey->AddNumber("player_level", GetConfig().Level());
    ranksKey->AddNumber("player_cur_xp", GetConfig().Xp());

    // ===== ДОБАВЛЕНО: сохранение дня бонуса =====
    ranksKey->AddNumber("last_bonus_day", GetConfig().LastBonusDay());
    ...
    Platform::Print("SaveRanksToConfig: Ranks, levels, and XP saved successfully!\n");
}

// ===== ТОЧНАЯ МЕХАНИКА CS:GO (ОБРАБОТЧИК КОНЦА МАТЧА) =====
void ClientGC::OnMatchEnd(GCMessageRead &messageRead)
{
    ...
    // (В реальном коде нужно проверять winner_side или winner_team)
    m_matchWonThisRound = true;

    // Получаем ссылку на память (без const_cast!)
    GCConfig &config = GetConfig();

    // ===== 1. ЗВАНИЯ (СОРЕВНОВАТЕЛЬНЫЕ РАНГИ) =====
    if (m_matchWonThisRound)
    {
        int compWins = config.CompetitiveWins() + 1;
        int wingWins = config.WingmanWins() + 1;
        int dzWins = config.DangerZoneWins() + 1;

        int compRank = config.CompetitiveRank();
        if (compWins % WINS_PER_RANK == 0 && compRank < 18) compRank++;

        int wingRank = config.WingmanRank();
        if (wingWins % WINS_PER_RANK == 0 && wingRank < 18) wingRank++;

        int dzRank = config.DangerZoneRank();
        if (dzWins % WINS_PER_RANK == 0 && dzRank < 15) dzRank++;

        config.SetCompetitiveRank(static_cast<RankId>(compRank));
        config.SetCompetitiveWins(compWins);
        config.SetWingmanRank(static_cast<RankId>(wingRank));
        config.SetWingmanWins(wingWins);
        config.SetDangerZoneRank(static_cast<DangerZoneRankId>(dzRank));
        config.SetDangerZoneWins(dzWins);
    }

    // ===== 2. РАНГ АККАУНТА (XP) =====
    int xpGain = m_matchWonThisRound ? XP_WIN : XP_LOSE;

    // ===== 3. ЕЖЕДНЕВНЫЙ БОНУС XP =====
    // Получаем текущий день года (1-365)
    time_t now = time(nullptr);
    struct tm* local = localtime(&now);
    int currentDay = local->tm_yday;

    int bonusXp = 0;
    // Бонус даётся только если:
    // 1. Мы выиграли матч
    // 2. Сегодняшний день не совпадает с днём последнего бонуса
    if (m_matchWonThisRound && config.LastBonusDay() != currentDay)
    {
        bonusXp = XP_DAILY_BONUS;
        config.SetLastBonusDay(currentDay);
        Platform::Print("Daily bonus XP awarded! +%d XP\n", bonusXp);
    }

    // ===== 4. ОБНОВЛЯЕМ XP И УРОВЕНЬ =====
    int newXp = config.Xp() + xpGain + bonusXp;
    int newLevel = config.Level();
    ...
    // ===== 5. СОХРАНЯЕМ ВСЁ В ФАЙЛ =====
    SaveRanksToConfig();

    // ===== 6. ОТПРАВЛЯЕМ ОБНОВЛЕНИЕ КЛИЕНТУ =====
    SendRankUpdate();

    Platform::Print("Match result: Win=%d, XP=%d, Bonus=%d, Total XP=%d, Level=%d\n",
        m_matchWonThisRound, xpGain, bonusXp, newXp, newLevel);
